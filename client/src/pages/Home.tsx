        ...
        {selectedScreen && (
          <div className="lightbox" role="dialog" aria-modal="true" aria-label={selectedScreen.title} onClick={() => setSelectedScreen(null)}>
            <div className="lightbox-panel" onClick={(event) => event.stopPropagation()}>
              <div className="lightbox-toolbar"><div><span className="lightbox-kicker">ExpenseTracker / schermata app</span><h3 key={selectedScreen.src} className={`lightbox-title ${lightboxDirection === 1 ? "lightbox-title-from-right" : lightboxDirection === -1 ? "lightbox-title-from-left" : ""}`}>{selectedScreen.title}</h3></div><button className="lightbox-close" type="button" onClick={() => setSelectedScreen(null)} aria-label="Chiudi immagine ingrandita">×</button></div>
